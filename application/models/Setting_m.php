	...
	function simpan() {
		...
		foreach ($opsi_val_arr as $key => $val) {
			if($this->input->post($key) || $this->input->post($key) == 0 ) {
				// str_replace('https://preview.ibb.co/e9Aqxd/', base_url('assets/email/'), $this->input->post($key));
				// $data = array ('setting_val'=> $this->input->post($key));
				$data = array ('setting_val'=> str_replace('https://preview.ibb.co/e9Aqxd', site_url('assets/email/'), $this->input->post($key)));
