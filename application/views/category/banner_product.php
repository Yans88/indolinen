	...
	<table id="example88" class="table table-bordered table-striped">
		...
		<tbody>
			<?php 
				...
				if(!empty($category)){		
					foreach($category as $c){	
						$view_sub = '';
						$path = !empty($c['img']) ? base_url('uploads/products/'.$c['img']) : base_url('uploads/no_photo.jpg');
						$info = $c['id_img'].'Þ'.$path;
						echo '<tr>';
						echo '<td align="center">'.$i++.'.</td>';
						
						echo '<td class="first" align="center"><a class="" href="'.$path.'" title="'.$judul_utama.'"><img width="790" height="500" src="'.$path.'"></a></td>';
			...
			<a href="#" id="'.$info.'" title="Edit" class="edit_category"><button class="btn btn-xs btn-success"><i class="fa fa-edit"></i> Edit</button></a>
				<button title="Delete" id="'.$c['id_img'].'" class="btn btn-xs btn-danger del_category"><i class="fa fa-trash-o"></i> Delete</button>		
						</td>';
						echo '</tr>';
					}
				}
...
$('.yes_save').click(function(){
	...
	var url = '<?php echo site_url('product/simpan_img');?>';
