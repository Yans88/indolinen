...
$nama = !empty($member->nama) ? $member->nama.' '.$member->surname : '';
...
$img = !empty($member->photo) ? base_url('uploads/members/'.$member->photo) : base_url('uploads/no_photo.jpg');
?>
...
<div class='alert alert-info alert-dismissable' id="success-alert">
	...
	<table class="table table-bordered table-reponsive">
		<tbody><tr class="header_kolom">
			...
			<th style="vertical-align: middle; text-align:center">Info Member</th>
	...
	<table class="table table-bordered table-reponsive">
		<tbody><tr class="header_kolom">
			...
			<th style="vertical-align: middle; text-align:center; width:33%;" >History Tier</th>
			...
			<td style="border:none;">
                ...
                <div class="box-body">
                  ...
                  <div class="direct-chat-messages">
					...
					$date_point = '';
					$i=0;
					foreach($history_tier as $ph){
						$date_point = '';
						$diskon = '';						
						$nama_tier = '';						
						$diskon = $ph['diskon'] > 0 ? $ph['diskon'].'%' : '-';
						$nama_tier = $ph['nama_tier'] != '' ? $ph['nama_tier'].' - '.$diskon : '-';
						$date_point = date("d-M-y H:i", strtotime($ph['update_at']));
						
						echo '<div class="direct-chat-info clearfix">';
						if($i%2){
							echo '<span class="direct-chat-text" style="font-size:12px; background:#F9F9F9; border:1px solid #F9F9F9;">Tier : '.$nama_tier.'<br/>User : '.$ph['fullname'].'<span style="float:right;">'.$date_point.'</span></span>';
						}else{
							echo '<span class="direct-chat-text">Tier : '.$nama_tier.'<br/>User : '.$ph['fullname'].'<span style="float:right;">'.$date_point.'</span></span>';
						}
						echo '</div>';
						$i++;
					}
		...
		<tr style="vertical-align:middle; text-align:left">
			...
			<td>
				...
				<thead><tr>
					...
					<th style="text-align:center; width:16%">Nama Alamat</th>			
					<th style="text-align:center; width:20%">Penerima</th>		
					...
					<?php 
						...
						if(!empty($_alamat)){
							foreach($_alamat as $_a){
								...
								echo '<td>'.$_a['nama_penerima'].' '.$_a['surname'].'<br/>Phone : '.$_a['phone'].'</td>';
								echo '<td>'.$address.'</td>';
								// echo '<td>'.$note.'</td>';
