			...
			<?php 
				...
				if(!empty($transaksi)){		
					foreach($transaksi as $t){						
						...
						$_text = 'Waiting approval';
						if(empty($t['confirm_date']) && empty($t['confirm_bank']) && empty($t['confirm_sender']) && empty($t['confirm_rek'])) {
							$_disabled ='disabled';
							$_text = 'Waiting payment';
						}
						
						if($t['status'] == 0 && $t['payment'] == 3){
							$_status = '<small class="label label-warning"><strong>'.$_text.'</strong></small>';
							$_action .= '<button title="Confirm" id="'.$_info.'" class="btn btn-xs btn-warning btn_confirm" '.$_disabled.'><i class="glyphicon glyphicon-question-sign"></i> Confirm </button>';
						}
						if($t['status'] == 0 && $t['payment'] != 3){
						...
						if($t['status'] == 3){
							...
							$_action .= '<button style="margin-top : 5px;" title="Dikirim" id="'.$info.'" class="btn btn-xs btn-success btn_send"><i class="glyphicon glyphicon-send"></i> Dikirim</button>';
						...
						if($t['status'] == 4){
							...
							$_action .= '<button style="margin-top : 5px;" title="Sampai tujuan" id="'.$info.'" class="btn btn-xs btn-success btn_sampai"><i class="fa fa-caret-square-o-down"></i> Sampai tujuan</button>';
						}
						if($t['status'] == 5){
						...
						if($t['status'] == 7){
							$_status = '<small class="label label-danger"><strong>Cancel Payment</strong></small>';
						}
						if($t['status'] == 8){
							$_status = '<small class="label label-danger"><strong>Complain</strong></small>';
						}
						if($t['status'] == 9){
							$_status = '<small class="label label-default"><strong>Pesanan selesai</strong></small>/<small class="label label-danger"><strong>Complain Rejected</strong></small>';
						}
						...
						echo '<td>'.date("d M Y H:i", strtotime($t['created_at'])).'<br/>'.$_status.'</td>';
												
						echo '<td>'.$t['nama_member'].' - '.$t['phone_member'].'<br/> Email : '.$t['email_member'].'</td>';	
						if($t['payment'] == 3 && $t['status'] > 0){
							echo '<td>Payment : '.$payment.'<br/>Total : <a href="#import_dialog" title="View data payment" id="'.$_info.'">'.number_format($t['ttl_all'],2,',','.').'</a></td>';
						}else{
