          ...
          <div class="modal-dialog" style="width:600px">
            <div class="modal-content">
              <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">×</span></button>
                <h4 class="modal-title">Add/Edit Banner</h4>
              </div>
	...
	<table id="example88" class="table table-bordered table-striped">
		...
		<tbody>
			<?php 
				...
				if(!empty($category)){		
					foreach($category as $c){	
						$view_sub = '';
						$path = !empty($c['img']) ? base_url('uploads/banner/'.$c['img']) : base_url('uploads/no_photo.jpg');
						$info = $c['id_banner'].'Þ'.$path;
						echo '<tr>';
						echo '<td align="center">'.$i++.'.</td>';
						
						echo '<td class="first" align="center"><a class="" href="'.$path.'" title="Banner"><img width="790" height="500" src="'.$path.'"></a></td>';
			...
			<a href="#" id="'.$info.'" title="Edit" class="edit_category"><button class="btn btn-xs btn-success"><i class="fa fa-edit"></i> Edit</button></a>
				<button title="Delete" id="'.$c['id_banner'].'" class="btn btn-xs btn-danger del_category"><i class="fa fa-trash-o"></i> Delete</button>		
						</td>';
						echo '</tr>';
					}
				}
...
$('.yes_del').click(function(){
	var id = $('#del_id').val();
	var url = '<?php echo site_url('banner/del');?>';
	$.ajax({
		data : {id : id},
		url : url,
		type : "POST",
...
$('.yes_save').click(function(){
	...
	var url = '<?php echo site_url('banner/simpan_cat');?>';
