...
<div class='alert alert-info alert-dismissable' id="success-alert">
	...
	<table id="example88" class="table table-bordered table-striped">
		<thead><tr>
            ...
            <th style="text-align:center; width:10%">Weight(Gram)</th>
		...
		<tbody>
			<?php 
				...
				if(!empty($product)){		
					foreach($product as $p){	
						...
						$diskon = '';
						$path = '';
						$view_img = site_url('product/banner_product/'.$p['id_product']);
						$path = !empty($p['img']) ? base_url('uploads/products/'.$p['img']) : base_url('uploads/no_photo.jpg');
						$diskon = (int)$p['diskon'] > 0 ? $p['diskon'].'%' : 0;
						echo '<tr>';
						echo '<td align="center">'.$i++.'.</td>';
						echo '<td>'.$p['nama_barang'].'<br/>Kategori : '.$p['nama_kategori'].'<br/>Sub Kategori : '.$p['nama_sub'].'<br/>Sub Sub Kategori : '.$p['nama_sub_s'].'</td>';
						echo '<td align="right">'.$diskon.'</td>'; 
						echo '<td align="right">'.number_format($p['qty'],0,',','.').'</td>'; 
						echo '<td align="right">'.number_format($p['weight'],0,',','.').'</td>'; 
			...
			<a href="'.site_url('product/add/'.$this->converter->encode($p['id_product'])).'" id="'.$info.'" title="Edit" class="edit_news"><button class="btn btn-xs btn-success" ><i class="fa fa-edit"></i> Edit</button></a>
			<button title="Delete" id="'.$p['id_product'].'" class="btn btn-xs btn-danger del_news" style="margin-top:2px;"><i class="fa fa-trash-o"></i> Delete</button><a href='.$view_img.' title="View"><button title="View more image" class="btn btn-xs btn-primary" style="margin-top:4px;"><i class="fa fa-tags"></i> View more image</button></a>	
