...
<div class="modal fade" role="dialog" id="frm_user">
            ...
            <div class="modal-content">
              ...
              <div class="modal-body" style="padding-bottom:2px;">
				...
				<div class="form-group">
					<input type="hidden" value="" name="id_color" id="id_color">
                  <label>Color</label><span class="label label-danger pull-right color_error"></span>
                  <input type="text" class="form-control" name="_color" id="_color" value="" placeholder="Color" autocomplete="off" />
                </div>	
                </div>
 ...
 <div class="box-header">
    <a href="#"><button class="btn btn-success add_user"><i class="fa fa-plus"></i> Add Color</button></a>
</div>
<div class="box-body">
<div class='alert alert-info alert-dismissable' id="success-alert">
	...
	<table id="example88" class="table table-bordered table-striped">
		<thead><tr>
			...
			<th style="text-align:center; width:80%">Color</th>			
		...
		<tbody>
			<?php 
				...
				if(!empty($colors)){
					foreach($colors as $mp){
												
						$info = $mp['id'].'Þ'.$mp['color'];
									
						echo '<tr>';
						echo '<td align="center" style="vertical-align: middle;">'.$i++.'.</td>';
						echo '<td style="vertical-align: middle;">'.$mp['color'].'</td>'; 
...
$('.yes_del').click(function(){
	var id = $('#del_id').val();
	var url = '<?php echo site_url('colors/del');?>';
	$.ajax({
		data : {id : id},
		url : url,
		type : "POST",
...
$('.yes_save').click(function(){
	...
	$('.color_error').text('');	
	if(color <= 0 || color == '') {
		$('.color_error').text('Color harus diisi');
		return false;
	}
	var dt = $('#form_users').serialize();
	var url = '<?php echo site_url('colors/save');?>';
	$.ajax({
		data:dt,
		type:'POST',
		url : url,
		success:function(response){			
			...
			if(response == 'exist'){
				$('.color_error').text('Color sudah digunakan');
			}
